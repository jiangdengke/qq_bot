...
""";
    ...
    @Listener
    @ContentTrim
    @Filter(value = "(?i)^overtime\\s+G[1-3]\\s+\\d+(?:\\.\\d{1,2})?$", matchType = MatchType.REGEX_MATCHES)
    ...
    private static final Pattern SET_WITH_TYPE_P =
            Pattern.compile("^overtime\\s+set\\s+(G[1-3])\\s+(\\d{6})\\s+(\\d+(?:\\.\\d{1,2})?)$", Pattern.CASE_INSENSITIVE);
    private static final Pattern SET_NO_TYPE_P =
            Pattern.compile("^overtime\\s+set\\s+(\\d{6})\\s+(\\d+(?:\\.\\d{1,2})?)$", Pattern.CASE_INSENSITIVE);
    ...
    public void query(OneBotGroupMessageEvent event) {
        ...
        try {
            ...
            String msg = "📊 本月合计：" + fmt(s.getMonthTotal()) + "h（" + byType + "）\n"
